...
export const validateImageFile = (
  ...
  // Check extension fallback if mime type is missing
  // (common on some mobile/HEIC browsers)
  const ext = (file.name || '')
    .split('.')
    .pop()
    ?.toLowerCase();
  ...
  if (!isTypeValid) {
    return {
      ...
      error: `Unsupported file type "${
        file.name || 'image'
      }". Please upload JPG, PNG, WEBP, or HEIC images.`,
    };
  }
  ...
  if (file.size > maxSizeBytes) {
    ...
    return {
      ...
      error: `"${
        file.name || 'Image'
      }" is ${sizeMb}MB. Maximum allowed image size is ${maxSizeMb}MB.`,
    };
  }
...
/**
 * Optimizes/compresses an image file client-side
 * to ensure fast loading and reliable storage.
 */
 ...
 *
 * Storage path:
 * {authenticated_user_id}/{folder_id}/{unique_filename}
 *
 * Uses a permanent public URL when the bucket is public.
 * Falls back to local data URL if Supabase upload fails.
 */
export async function uploadFileToVaultStorage(
  ...
  const uniqueFilename =
    `${timestamp}-${randomSuffix}.${cleanExt}`;
  ...
  // {unique_filename}
  const storagePath =
    `${authUserId}/${folderId}/${uniqueFilename}`;
  ...
  if (supabase) {
    try {
      ...
      // NOTE:
      // vault-photos must be a PUBLIC bucket
      // in Supabase Storage for this URL to work.
      const {
        data: publicUrlData,
      } =
        supabase.storage
...
export async function uploadMemoryPhotoToStorage(
  ...
  return uploadFileToVaultStorage(
    file,
    memoryId,
    userId,
    'vault-photos',
...
/**
 * Synchronize the public.memory_photos table
 * in Supabase with the current ordered photo URLs.
 */
