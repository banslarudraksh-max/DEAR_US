...
VALUES 
  ...
  ('vault-photos', 'vault-photos', false),
...
-- Vault photos, memories, and audio: STRICTLY AUTHENTICATED ONLY
DROP POLICY IF EXISTS "Authenticated users can view vault assets" ON storage.objects;
CREATE POLICY "Authenticated users can view vault assets" ON storage.objects
  FOR SELECT TO authenticated
  USING (bucket_id IN ('vault-photos', 'memories', 'audio'));

-- 15.2 STORAGE INSERT POLICIES
DROP POLICY IF EXISTS "Authenticated users can upload vault assets" ON storage.objects;
CREATE POLICY "Authenticated users can upload vault assets" ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (
...
CREATE POLICY "Authenticated users can update own vault assets" ON storage.objects
  FOR UPDATE TO authenticated
  USING (
    bucket_id IN ('vault-photos', 'memories', 'avatars', 'audio')
...
CREATE POLICY "Authenticated users can delete own vault assets" ON storage.objects
  FOR DELETE TO authenticated
  USING (
    bucket_id IN ('vault-photos', 'memories', 'avatars', 'audio')
    AND (auth.uid() = owner OR public.is_admin())
